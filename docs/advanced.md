...
服务器强制：回复 To 取自原信；转发目标必须你指定且在白名单；无 BCC；滚动一小时最多 5 封（记在磁盘上）；验证码信不能读、回、转。没有确认卡会返回 `CONFIRMATION_UNSUPPORTED`，不会发。不要用 `QQCONNECT_SEND_UNSAFE_NO_CONFIRM` 绕过，除非在跑本仓库测试。
