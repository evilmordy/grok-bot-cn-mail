...
3. SMTP send waits for MCP elicitation via `inputRequired` (SDK 2025 shim / 2026 retry). Do not call `elicitInput` inside the tool handler: Grok Bot Auto-review is permission for the tool, and a nested `elicitation/create` is declined as `send cancelled`. **Accept sends; Decline aborts.** If the client cannot elicit, `CONFIRMATION_UNSUPPORTED`. `QQCONNECT_SEND_UNSAFE_NO_CONFIRM=1` is a documented foot-gun for tests, not a default.
